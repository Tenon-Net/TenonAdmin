...
2. **Layer 2 - `omx hud` CLI command**: Shows runtime orchestration state and turns. Legacy names in its output identify existing records for inspection, not workflows to activate.

Use Layer 2 only on a supported attached OMX CLI surface. In Codex App outside tmux, inspect supported native status/configuration instead of launching or fabricating a HUD.
...
- Do not start a workflow merely to populate the HUD; report the idle state.
