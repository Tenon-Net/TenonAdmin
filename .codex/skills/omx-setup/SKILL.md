...
Use this task card to install or refresh OMX for the current project and/or user scope. Shared safety, lifecycle, hook, team, autonomy, and cancellation rules live in [AGENTS.md](../../../AGENTS.md). Do not infer new CLI behavior from this card; the implementation is authoritative.
...
If using local source, run `npm run build` before `node dist/cli/omx.js setup --force --verbose` and `node dist/cli/omx.js doctor`. A preserved customized AGENTS.md is not a failure. Use `--force` or `--merge-agents` only when the requested refresh requires replacing managed content, after inspecting its dry-run and preserving customizations; do not edit lifecycle state manually.

## Discovery troubleshooting

If `$omx-setup` is absent/stale: run `omx setup --verbose`, then `omx doctor`; verify `./.codex/skills/omx-setup/SKILL.md` for project scope or `${CODEX_HOME:-~/.codex}/skills/omx-setup/SKILL.md` for legacy user scope. In plugin mode verify the oh-my-codex plugin is installed/discovered. For duplicates, inspect overlapping skill names across the standard `.agents/skills` and OMX `.codex/skills` roots; retain one Codex entry per name and preserve unrelated skills.
