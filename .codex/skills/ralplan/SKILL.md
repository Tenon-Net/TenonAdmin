---
name: ralplan
description: "[OMX] Build a consensus plan with sequential Planner, Architect, and Critic reviews before execution."
---
...
This card owns consensus; `$plan` is lightweight planning and has no consensus mode.

## Execution follow-up

After the durable, matching execution handoff exists, load the selected consumer: `$ultragoal` for durable execution; `$team` for separately authorized parallel work; `$autoresearch` for validator-gated research; `$performance-goal` for measured optimization. Preserve specialist roles and verification responsibilities. Retired entrypoints are not fallback execution lanes.
...
Hook classification routes requests; it does not grant implementation or consensus authority. Ground vague requests through the required clarification stage. A file path, issue number, `force:` prefix, or prompt length is not proof of completed review or permission to skip a gate.

Stop after terminal planning state and the required evidence are durable, or report the specific unavailable role/authority/review. Only a matching authorized handoff starts execution. Preserve the active branch when the user says continue; do not restart completed discovery.
