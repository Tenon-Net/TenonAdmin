...
- 当前阶段: review 已完成,P1/P2/P3 已入 `## Findings`;**射程限制 R1–R11 已按 P3-4 转写进 `## Findings`**(否则 Task 9 的 plan 一落笔就全丢)
- 上一轮: Round 31 — Task 8 **review**(`Agent(model="opus")`,不传 `subagent_type`,全工具含 Edit)。已 declare「自审」,报告 37.7KB 落 `scratchpad/review8-report.md`。**P1 0 条,P2 2 条,P3 5 条**。
  **有真实守门的部分(逐刀实证,不是嘴上说)**:D2 分类表逐区间(423 搬终态 → 只红 423 那行;404 搬重试 → 红 2 行;3xx 挪走 → 红)、事务探针**反向验证**(把「无事务」断言改成断「有事务」→ 转红,证明探针真在跑而不是挂错地方)、围栏映射换形状(保留 `catch` 只把 `TerminalFailure` 改成 `RetryableFailure` → 红 8 条,红在**结果类型**上而不只是「没抛」)、响应截断两个字面值 4096/512、header 落地、方法白名单与 `new HttpMethod` 的**先后顺序**(顺序颠倒 → `FormatException` 红 `BAD METHOD` 那行)、`ResolveTimeout` 上界与 deadline 各自转红。
  **⚠ 两处「没红」是本轮的实质产出**(详见 `## Findings` 的 P2-1/P2-2)。协调者**各用 review 没用过的第三种形状独立复验,两次都拿到更强的证据**:
  **M-COORD-C(验 P2-1)** —— review 用的是「删掉整条注册」与「换成 `Insert(0,…)`」两刀(都在动 `WorkflowSetup`);协调者改动 handler 自己:**把 `WebhookNodeHandler.NodeType` 从 `Webhook` 改成 `Approval`**(内置从此根本不是 Webhook handler)→ **红 2 条**(`The_webhook_handler_is_registered_and_resolvable` 与端到端的 `The_webhook_http_call_happens_outside_any_database_transaction`),**`A_pre_registered_handler_wins_over_the_built_in_one` 照样绿**。这比 review 的两刀更彻底:**W-DI2 连「内置到底是不是一个 Webhook handler」都看不见**,它断的完全是 Microsoft DI 的 `Insert(0,…)` 语义,与 `WorkflowSetup` 写了什么零关系。
  **M-COORD-D(验 P2-2)** —— review 用的是「删掉 4 个字段」;协调者改用**字段错位**:把 `tokenId = context.TokenId` 与 `nodeVisitId = context.NodeVisitId` **两个值对调** → **51/51 全绿**。这正是 Plan §4 W-Q1 声称要钉住的「字段接错」形状,实测**零守门**;而错位比缺字段更阴险——消费者接收端拿到的是**结构完整但语义颠倒**的 payload,不会解析失败,只会默默算错。
  两刀均先 `grep`/`sed -n` 确认落盘、跑完 `git checkout -- <单文件>` 精确复原,`git status` 回到仅未跟踪 `TestResults/`。
  **review 自身的纪律核查(协调者复核)**:①最终 `git diff HEAD --stat` **为空**,产品与测试代码逐字节回到 `39317cc`,**未 commit 未 push**;②禁碰清单未被触碰,`WorkflowReplaceabilityTests.cs` 仍 **10** 条 `[Fact]`;③最终闸门 **397/397 全绿**(过滤器一字未改);④它**如实核了 exec 的 13 个警告**(`--no-incremental` 全量重编译,按码归类 CS8602×4 / CS8601×1 / CS1574×6 / CS1573×2,全部来自 Services/Core,Workflow 与 Tests 各 0)——历轮该 exec 有过「0 warnings」的不准确自述,**这一次数字是准的**。
  **review 对 exec 自述的裁定**:四处偏离**全部属实**,但偏离③的**结论站不住**——「`Insert(0,…)` 如实复现了 D6/陷阱 11 的真实机制」不成立(它复现的是**结果顺序**,而那个顺序由容器保证)。**责任分摊到 plan**:Plan §4 H 组给 W-DI2 写的预期变异(「`TryAddEnumerable` 改 `Add` 且顺序颠倒 → 红」)本身就是错的,exec 是照着一份有缺陷的 plan 实现的。这不改变它是 P2 的事实。
  另:review 指出 exec 报告 M-FENCE-ESCAPE「红 8 条」但只列了 7 个名字(漏列 `On_failure_manual_also_covers_config_failures`)——**数字对,清单少一行**,记为轻微不准确,不构成发现。
- 下一步: Round 32 — Task 8 **修 Findings**。协调者派 `Agent(subagent_type="oh-my-claudecode:executor", model="sonnet")`,**只修 P2-1 与 P2-2 两条**,两条的修法 review 都已**实跑验证过**(P2-1 的替换代码块可直接抄,已验证编译通过、基线绿、且对 M-DI-DELETE 与 M-DI-FIRST 两刀均转红;P2-2 是补 4 行 `Assert.Equal`)。**给 exec 的硬约束**:①**条数不许变**——P2-1 是**改写**已有的 `A_pre_registered_handler_wins_over_the_built_in_one`(不新增方法,`[Fact]` 仍 22、总数仍 **397**),P2-2 是在已有方法里**补 4 行断言**(不新增方法);②**禁碰 `WorkflowReplaceabilityTests.cs`**(必须仍 10 条)、禁碰 `WfNodeExecutionDispatcherTests.cs` 与三个 Store/三个实体/`WorkflowEngine.cs`/`WfNodeExecutionDispatcher.cs`/`EnterNodeOp.cs`/`Directory.Packages.props`/任何 `.csproj`;③**产品代码一行不许动**——两条 P2 都是「测试没跟上」而非「产品写错」,`WebhookNodeHandler.cs`/`WorkflowSetup.cs` 本轮应零改动,若 exec 认为必须动产品代码,**先停下来申报理由**;④P3 **一律不修**(P3-1 的 `FixedTime` 改造、P3-2 的引擎侧 `Retry-After` 端到端、P3-5 都挂账,P3-2 还要改禁碰文件,须由后续 Task 的 plan 显式开口);⑤自验要求:改完后**自己把 P2-1 的两刀(删注册 / `Insert(0,…)` 前置)与 P2-2 的两刀(删 4 字段 / 对调 `tokenId`↔`nodeVisitId`)各跑一遍**,四刀都必须转红再复原,**尤其对调那一刀是协调者发现的、review 没做过的形状,必须覆盖**;⑥闸门两条重跑,**397 只增不减**,警告基线 **13** 不许涨,如实报数。协调者事后**亲自重跑四个变异点转红后复原 + 重跑闸门**,再决定勾选。
...
- [ ] **P2-1★｜`W-DI2` 对产品代码鉴别力为零,「消费者可替换内置 Webhook handler」这条产品承诺零守门**
...
- [ ] **P2-2｜外呼请求体(D5 契约)8 个字段里有 4 个零断言,且字段错位完全测不出**
