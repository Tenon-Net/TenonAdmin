...
- [ ] **P3-1(挂账)｜陷阱 14「必须用注入的 `TimeProvider`」零守门**:`M-TIMEPROVIDER`(把 `:217` 的 `time.GetUtcNow()` 换成 `DateTimeOffset.UtcNow`)→ 51/51 全绿。`NewHandler` 带 `TimeProvider? time = null` 形参,但 51 条**没有一条**传过非 `TimeProvider.System` 的值,注入点从未被行使。两种写法真实行为等价,危害只是「注释宣称的注入点其实没被验证」+ 慢机器上 `W-R2` 的理论 flake。**修法**:加仓内成法的 `FixedTime : TimeProvider`(`SoftDeleteAuditTests.cs:71` 等四处同形先例),`W-R2` 改用固定时钟后可把 `InRange(55s,65s)` 收紧成 `Assert.Equal(TimeSpan.FromSeconds(60), …)`(HTTP-date 只有秒精度,固定时钟下差值恒为整 60s)。
- [ ] **P3-2(挂账,归属后续 Task)｜「handler 吐出的 in-range `Retry-After` 真被引擎采用」全仓零覆盖**:`ResolveRetryDelay`(`WorkflowEngine.cs:1452-1459`)只在 `(0,24h]` 内采用它;现有两条相关测试(`WfNodeExecutionDispatcherTests.cs:392-410` / `:418-441`)喂的是 `TimeSpan.Zero` 与 `TimeSpan.FromDays(3650)`——**两个都在区间外,两条断的都是 fallback 值**。故若 `ResolveRetryDelay` 退化成无条件返回指数退避、彻底忽略 `RetryAfter`,这两条**仍然都绿**。**这是 Task 7 就存在的空白,不是 Task 8 新引入的**;补法极廉价(照既有姿势加第三条,喂 `TimeSpan.FromMinutes(7)` 断 `NextRetryAtUtc`),但**要改禁碰的 `WfNodeExecutionDispatcherTests.cs`,必须由 Task 9 或 Task 10 的 plan 显式开口**。不修的症状:对方明说 120s 后再来、我们 30s 就撞上去,只在对端限流日志里可见。
...
- [ ] **P3-1(挂 Task 10)｜E1 行数断言有效,但断言顺序会掩盖它**(`WfNodeExecutionContractTests.cs:49-71`)。把首个期望行数 1 改 2 会精确红(`Expected 2 / Actual 1`),所以不是死断言;但唯一索引被删时 `Assert.NotNull(failure)` 先抛,后面的 row-count 永远不执行。后续可先采集异常与计数、先断计数再断异常,提升失败诊断独立性。
...
- [ ] **P3-3(挂 Task 10,仅改测试注释)｜E12 的 SQLite 射程说明过宽**(`WfNodeExecutionContractTests.cs:238-241`)。SQLite 确实不能证明数据库侧宽度拒绝/截断,但能证明 C# 侧 `Truncate(...,512)` 守卫;把 Store 的 `Truncate` 变异为原样返回后,E12 在 `:259` 以 `Expected 512 / Actual 600` 转红。后续把注释拆成这两层,不要把整条测试写成 SQLite 无鉴别力。
