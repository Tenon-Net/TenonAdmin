...
- 轮次: 35
- max: 70
- 当前任务: Task 9(四库持久化契约套件)
- 当前阶段: review 已完成,3×P2 待修;Task 9 未勾选
- 上一轮: Round 35 — Task 9 **review**(Codex `code-reviewer` 独立自审 + 变异测试)。报告落 `C:\Users\Shiny\AppData\Local\Temp\codex\tenon-admin-e12351c7\review9-report.md`(**29,658 bytes**,SHA256 `6D919864B561977783638FA53B43B7AB62FB6BF99CFD49849ECF3BC3A8A0E534`)。结论 **REQUEST CHANGES:0×P1 / 3×P2 / 3×P3**,故 Task 9 **仍不勾选**。
...
- [ ] **P2-1｜E2 的 attempt 宽度/Unicode/hash 断言没有读回数据库,四库持久化契约只验了内存对象**(`WfNodeExecutionContractTests.cs:146-165`)。execution 行在 `:132-144` 会重新 query,但 `succeededAttempt`/`failedAttempt` 是 `AppendAsync` 构造并在 insert 后直接返回的对象(`WfNodeExecutionAttemptStore.cs:50-66`);若数据库截断、字符转换或列映射错位,现断言仍可能全绿。**修法**:按两个 attempt 的 `Id` 从 `db.Queryable<WfNodeExecutionAttempt>()` 读回,再对读回对象断 `OutputSummary`/`ErrorSummary`/`OutputHash` 与中文内容;测试条数不变,产品代码不动。
- [ ] **P2-2｜E7 删除 outbox 写入仍然 1/1 绿,「三表同事务写入再整体回滚」至少有一支为空转**(`WfNodeExecutionContractTests.cs:379-404`)。变异只删 `WfOutboxStore.EnqueueAsync` 两行,保留 execution/attempt/throw,专项仍退出 0;回滚后断 outbox 0 行无法区分「写过后回滚」与「压根没写」。attempt 支同形也缺正向证明。**修法**:故意 throw 前、事务仍健康时从库断 attempt 与 outbox 各恰好 1 行;throw 后继续在事务外断二者 0 行。修后分别删除 attempt append 与 outbox enqueue,两刀都必须因新增正向断言转红。
- [ ] **P2-3★｜E11 的 `PayloadVersion is 0 or 1` 是禁写清单点名的松散范围,不能暴露四库分歧**(`WfNodeExecutionContractTests.cs:605-616`)。SQLite 已两次独立实测确定读到 `0`;接受 0/1 同时放行「SqlSugar 未回填」与声明的 `DefaultValue=1`,使这条四库探针几乎失去鉴别力。**即时修法**:无条件改 `Assert.Equal(0, loadedHistory.PayloadVersion)`,明确它记录当前 SQLite 观测并故意让其他三腿在行为不同时时转红;**不要**按 DbType 分支成 SQLite=0/其他=1,否则会掩盖差异。永久口径与产品注释由 Task 10 根据四腿结果定案。
