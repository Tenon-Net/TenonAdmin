...
- 上一轮: Round 22 — Task 6 **exec**(executor/sonnet)。commit `37b83da`,**7 个文件 +1261/-1**,与 Plan §2 改动清单**文件级精确一致**(新增 `WfNodeExecutionDispatcher.cs` 185 行、`Operations/WfManualFallbackOp.cs` 53 行、`tests/WfNodeExecutionDispatcherTests.cs` 648 行;修改 `WfCommands.cs` +27 纯追加、`WorkflowEngine.cs` +336、`WfNodeExecutionAttemptStore.cs` +6/-1、`tests/WfFakeNodeHandler.cs` +7)。协调者**独立复核**:①`WfCommands.cs` 纯追加(`^-` 命中 **0**);②**全 commit 唯一那 1 行删除**查实为 `WfNodeExecutionAttemptStore.Truncate` 的 `private` → `public`,**行为零改动**,且新注释写明「供 Task 6 的 `wf_node_execution.Summary` 复用——截断规则不许在两处各写一遍(Task 5 P3-1 的教训)」,正是 Plan 要的那一条;③禁碰文件(`WorkflowSetup`/`ReplaceabilityTests`/`EnterNodeOp`/`WfEnums`/`Entities/`/两个 Store/`WorkflowErrorCode`/`web`/`docs`/`.github`)命中 **0**,十件套仍 **10** 条;④`AttemptCount + 1` / `Attempt + 1` 命中 **0**(差一陷阱未踩);⑤重跑两条闸门:build **0 错误**(Workflow 包自身贡献 **0** 警告),test **334/334 通过、失败 0**(322 + 12 精确吻合)。
  **⚠ 协调者查出一处与 Plan 字面不符、且 exec 自述不准确之处,留给 review 裁定(不由协调者定性)**:Plan §2/D5 与我的 exec prompt 都写死「**全仓只许新增一处** `Updateable<WfNodeExecution>`」,exec 报告也自称「D5 全仓唯一一处」——**但实际是两处**(`WorkflowEngine.cs:1348` 与 `:1365`)。查实二者**都在同一个方法 `ClaimExecutionWritebackAsync` 的 if/else 内**(`RetryScheduled` 分支写 `NextRetryAtUtc` + 清租约;终态分支写 `CompletedTimeUtc`/`ErrorCode`/`Summary`),**两条 `.Where` 谓词逐字相同且三条齐全**(`Id == executionId && e.Fence == fence && e.Status == WfNodeExecutionStatus.Running`),共用同一个 `affected != 1` → 抛 `executionFenceConflict`。**故那条规则要保护的不变量(不得存在第二个不受控的写入点、每处回写必带 fence)是守住的**,分歧只在字面。**但 exec 报告把它写成「唯一一处」是不准确的自述**,这一点必须记下来——本 loop 的纪律就是不采信子代理的自我报告。**留给 Round 23 review 判定**:①这是否算 Plan 偏离(若算,是否可接受);②两个分支的列集确实不同(SqlSugar 的 `SetColumns` 单表达式难以条件化),合并成一处是否反而更差(要在重试分支显式写 `CompletedTimeUtc = null`、终态分支写 `NextRetryAtUtc = null`);③**更要紧的是变异验证**:两条谓词是分别写的,意味着**可能只有一条分支的 fence 被测试覆盖**——review 必须对**两个分支各做一次**「删 `Fence`」与「删 `Status == Running`」的变异(共 4 次),确认 T2/T3 对**终态分支**与**重试分支**都有鉴别力,而不是只钉住了其中一条。
  另:exec 报告主动说明了两处 Plan 留白的实现选择(均非偏离,协调者认可):①测试脚手架**不经 HTTP** 发布定义/发起实例(直接插 `WfDefinitionVersion` 行 + 调 `StartInstanceCmd`),但**用户账号仍走 HTTP**(审批人解析需要真实 `SysUser` 行);②T8 需要「token 真实停在 node1 且该节点无 assignee」,而正常发起流程下无 assignee 的节点会被 `ApplyNobodyAsync` 直接放行、token 根本停不住,故改为**先用「有 assignee」的版本正常发起(token 真停、待办真建),再 UPDATE 这份已存在的真实版本行的 `ModelJson` 换成无 assignee 变体**——手法与语义契约 Task 3 定案的「直接 UPDATE 时间戳到过去」同源,操纵的是库里已存在的真实行而非伪造内存对象。
- 下一步: Round 23 — Task 6 **review**。协调者派 `Agent(model="opus"`,不传 `subagent_type`,全工具含 Edit)。要求 declare「自审」,报告落 `scratchpad/review6-report.md` 再回一句话。**变异着力点**:(a) **DONE-CONDITION 两条不变量优先**——把 `handler.ExecuteAsync` 挪进 tx1 的 `UseTranAsync` 闭包 → **T1 必须红**;(b) **上面那处两分支 CAS,必须四个变异分别做**(终态分支删 `Fence`、终态分支删 `Status == Running`、重试分支删 `Fence`、重试分支删 `Status == Running`)→ 确认 T2/T3 对**两个分支都有鉴别力**,若某个分支的某条谓词删掉后全绿,那就是一个真缺口;(c) `WfManualFallbackOp.ExecuteAsync` 的空人早返回换成 `await EnterApprovalAsync(ctx, ct)` → **T8 必须红**(这是本里程碑最危险的静默 bug:自动节点执行失败后被自动放行);(d) `WfManualFallbackOp` 换成 `new EnterNodeOp(node)` → T7 应红(`NodeVisitId` 被重新生成);(e) `RetryScheduled` 分支不写 `NextRetryAtUtc` → T4 应红;(f) `ResolveRetryDelay` 去掉上下界钳制 → T6 **两段都应红**;(g) 预算判定 `>=` 改 `>` → T5 应红;(h) 删掉 `ResolveExecutionOutcome` 的实例状态前置判定 → T9 应红。**另需重点审**:①12 条测试有没有 Task 4/5 那种「断言缺失」型空转(**本轮尤其查:`HandlerType`/`Summary`/`ErrorCode`/`CompletedTimeUtc` 这四个新填的列有没有被从库读回验证过**,Task 5 的 P2-1 就是「主契约列从没被读回」);②`Summary` 是否真的复用了 `Truncate` 而没有第二份截断;③禁写清单 11 条有没有被变相违反;④射程限制 R1–R10 是否如实(尤其 R1「生产侧零调用点」不许被偷偷「补上」)。
  **⑤exec 在完成通知里自己标注了两处「值得注意的细微处」,协调者判断这两条都指向潜在缺口,review 必须专门查**(exec 已把细节写进 `exec6-report.md`):
  **(i) T3 的转红是以「唯一索引冲突」的形式出现,而不是干净的 48004。** 这正是 Plan §5 陷阱 2 预言过的形状:若 CAS 晚于 `AppendAsync`,老 owner 会用被推高的 `AttemptCount` 插 attempt,撞 `uk_wf_node_exec_attempt_no`,**症状伪装成唯一键 bug 而非 fence 过期**。T3 是 DONE-CONDITION「同一 `ExecutionKey` 只推进一次」的两条守门测试之一,**若它在变异下是靠唯一索引抛异常才红的,那它可能是「因为错误的原因而红」**——而 Plan §4 禁写清单第 4 条明写「『被拒绝』的测试不许只断言异常,必须同时断言正向状态未变与副作用行数为 0」。review 要判定:T3 现在到底钉住了什么?**在删 `Status == Running` 的变异下,除了异常,它的「attempt 仍 1 行 / outbox 仍 1 行 / token 未再前进」这几条断言是否真的执行到并生效了?** 若异常在断言之前抛出、导致那几条断言压根没跑,那这条测试对「重复推进」的证明力就是虚的。
  **(ii) T6 是被下界那一段抓住的,上界那一段没有独立鉴别力。** exec 报告称去掉上下界钳制后 T6 转红,但红的是 `RetryAfter = TimeSpan.Zero` 那一段;这意味着**上界(24h 封顶)可能删掉也没人发现**。`RetryAfter` 是 handler(消费者代码)提供的 trust boundary,Plan D4 明写上界钳制「是必须实现的,不是优化」(`TimeSpan.FromDays(3650)` 会让 `nowUtc + delay` 逼近 `DateTime.MaxValue`,四库列写入行为各不相同)。review 要**单独变异上界**(只去掉 `> 24h` 那一支、保留下界)并确认是否有测试转红;若没有,这是一个真缺口(修法:把 T6 拆成两条,或在上界段加独立断言)。
  **仍不勾选**。
