...
- 当前阶段: exec 已完成,后端两条 + 前端三条闸门已由协调者独立复跑
- 上一轮: Round 30 — Task 8 **exec**(executor/sonnet)。commit `39317cc`,**7 个文件 +967/-1**,与 Plan §2 改动清单**文件级精确一致**(新增 `Providers/WebhookNodeHandler.cs` 268 行、`tests/WfWebhookNodeHandlerTests.cs` 627 行;修改 `Schema/WfNode.cs` +35/-1、`Abstractions/WorkflowErrorCode.cs` +9、`WorkflowSetup.cs` +10、双模板 `schema.d.ts` 各 +9 生成物)。条数 **346 → 397**(新增 **51** 条 = 22 `[Fact]` + 3 `[Theory]` 共 29 行),落在 Plan §4 预估的 385–400 区间内。
  协调者**独立复核,不采信子代理自报**:①**禁碰清单命中 0** —— `WfNodeExecutionDispatcher.cs`/`WorkflowEngine.cs`/`EnterNodeOp.cs`/三个 Store/三个实体/`IWorkflowNodeHandler.cs`/`JobHttpFence.cs`/`JobHttpClient.cs`/`ServicesSetup.cs`/`WorkflowReplaceabilityTests.cs`/`WfNodeExecutionDispatcherTests.cs`/`WfFakeNodeHandler.cs`/`WfNodeHandlerContractTests.cs` **全部不在 commit 的 7 个文件里**;②**D1 硬约束实测守住** —— `TenonAdmin.Workflow.csproj` 逐字读过,只有 `FrameworkReference Microsoft.AspNetCore.App` + 一条 `ProjectReference` 到 AspNetCore,**零 `PackageReference`**,`Directory.Packages.props` 未在 commit 内;`WorkflowSetup` 那条注册是 `sp.GetRequiredService<TenonAdmin.Services.JobHttpClient>().Client`,**复用内核单例而非新建客户端**;③**D4 裁决守住** —— 全 commit 无 `HostedService`/`IAdminJob`/`sys_job` 种子行,`EnterNodeOp.cs` 一个字节没动;④`WfNode.cs` 那 **1 行删除**查实是把注释 `// ── webhook (M3) ──` 改写成 `// ── webhook (M3a-1 Task 8) ──`,**`WebhookUrl` 属性一个字未改**(向后兼容);⑤双模板 `schema.d.ts` **SHA256 完全一致**(`9b129a31…`),diff 形状是纯生成物追加(4 个 props + `WfWebhookFailureAction` 枚举),无手改痕迹;⑥`[Fact]` 计数:十件套仍 **10**,`WfNodeExecutionDispatcherTests` 仍 **24**(Task 7 的 346 条一条未动)。
  **闸门(协调者亲自重跑,五条全绿)**:build **0 错误**(本次为增量缓存故显示 0 警告;exec 用 `--no-incremental` 全量核实为 **13** 个,全部是 Services/Core 既有的 `CS1573`/`CS1574`/`CS8601`/`CS8602`,Workflow 包自身 **0**,与基线持平);test **397 通过 / 0 失败 / 0 跳过**(1m53s);前端 `npm run typecheck` **零输出通过**、`npm run lint` **零输出通过**、`npx vitest run src/workflow/` **3 文件 35 条全过**。**`check-contract-drift.mjs` 协调者未重跑** —— 它会自起 Development `MinimalHost`,而项目记忆里「真实 `admin.db` 绝不碰」是硬线;改用**两条等效证据**替代:双模板 SHA256 逐字节相同 + diff 形状纯为新字段追加,足以证明 `schema.d.ts` 出自 `gen:api` 而非手改(exec 侧已跑过并报 `contract in sync`,此处按纪律记为**子代理自报、协调者以另一种证据佐证**)。
  **协调者亲自跑的两个变异都刻意换成 exec 没用过的形状,且第一个直接解决了 exec 自己申报的疑点**:
  **M-COORD-A(★ 针对 exec 申报的偏离①)** —— exec 报告 M-OCE-BOUNDARY(删 `when (!cancellationToken.IsCancellationRequested)`)下**只有「飞行中取消」那条转红**,「进门前预取消」那条免疫,原因是它自己加了一行 plan 未点名的 `cancellationToken.ThrowIfCancellationRequested()`;它如实记了账,但**没有回答那行新代码本身有没有测试守门**。协调者补上这一刀:**保留 `when` 过滤器不动,只删掉那行入口检查** → **恰好 1 条转红**,正是 `An_externally_cancelled_call_lets_the_cancellation_escape`,且红在它的**第一条断言 `Assert.Equal(0, transport.SendCount)`**(禁写清单第 4 条要求的「副作用先断」在这里真的兑现了)。**结论:两条取消路径各有一条测试独占守门、互不重叠,不是缺口而是分工** —— exec 申报的「鉴别力偏移」经此测定为**无害**,该记为已澄清,review 不必再查这一条。
  **M-COORD-B** —— 不用 exec 的 M-501(改分类表),而改 `ApplyFailureAction` 的守卫:把 `result.Type != TerminalFailure` 换成 `result.Type == Succeeded`(即让 `manual` 顺手把 `RetryableFailure` 也吞成 `ManualFallback`——一个看着很像「兜底就该兜全」的合理优化,却会让 503 这类瞬时故障永不重试) → **恰好 1 条转红**:`On_failure_manual_does_not_touch_retryable_results`。证明 G 组第三条不是陪跑,`manual` 与重试的边界有独立断言钉死。
  两次变异均先 `grep`/`sed -n` 确认落盘、跑完后由备份文件精确复原,`git status` 回到**仅未跟踪 `TestResults/`** 的干净态。
  **exec 另申报 3 处偏离(均不改契约,协调者转记待 review 复核)**:②`W-R2` 用 `Assert.InRange(TimeSpan,TimeSpan,TimeSpan)` 代替不存在的 `Assert.Equal(…, precision)` 重载(编译期 `CS1503`);③`W-DI2` 用 `services.Insert(0, …)` 而非 `AddScoped`,理由是 `ConfigureTestServices` 跑在 `AddTenonAdminWorkflow` 之后、`FirstOrDefault` 挑先注册的那个(`ReplaceabilityTests.cs:63` 有同款先例);④请求体/Content-Type 在 `FakeTransport` 回调**内部**捕获,因为 handler 用 `using(request)` 在返回前就 Dispose 了 `Content`。
- 下一步: Round 31 — Task 8 **review**。协调者派 `Agent(model="opus"`,不传 `subagent_type`,全工具含 Edit)。要求 declare「自审」,报告落 `scratchpad/review8-report.md` 再回一句话。**变异着力点**:(a) **B 组那条 22 行 `[Theory]` 是本 Task 的头号交付物**(D2 分类表的唯一守门人),要**逐区间**单独变异——408/423/425/429 各挑一个搬进终态区、把某个 4xx 搬进重试区、把 3xx 从终态挪走,确认**每一行都有独立鉴别力**而不是靠 501 一行撑门面;(b) **`Retry-After` 直传是否真的没被 `ResolveRetryDelay` 的 `(0,24h]` 钳制悄悄改写** —— C 组三条只测了 handler 自己吐的 `RetryAfter`,要确认「handler → dispatcher → 引擎」这一路上该值不被截断/丢弃,若无端到端覆盖如实记 P3;(c) **E 组 8 条共享同一个 `catch (AdminException)`**(exec 的 M-FENCE-ESCAPE 一刀红 8 条),要换形状:**保留 catch 但把 `TerminalFailure` 改成 `RetryableFailure`**,确认 8 条红的是**结果类型**而不只是「没抛异常」;(d) **I 组那条「socket 打开时无事务」是 DONE-CONDITION 明文** —— 用 Task 6 T1 同款手法反向验证:把探针的 `db.Ado.IsAnyTran()` 断言改成断 `true`,确认它确实在跑而不是探针挂错了地方;(e) `ResolveTimeout` 的 `[1,120]` 钳制与 deadline 参与,三个边界各单独变异。**另需重点审**:①51 条里有没有 Task 4/5/6 那种「**断言缺失**」型空转,**尤其查响应截断的两个字面值(4096/512)与 header 落地那条是否真从出站请求读回**;②`Assert.NotNull` 钉时间/对象的形状(Task 6 P2-1 原形)有没有再犯;③**松散边界代替量值断言**(Task 7 P2-2 原形)——`W-R2` 的 `InRange(55s,65s)` 是否够紧;④对外契约用常量自证而非字面值的老病(48029/48030/48031 三处);⑤`WorkflowSetup` 那条注册的**可替换性**是否真成立(消费者前置同 `NodeType` 的实现能否胜出——`W-DI2` 只用 `Insert(0,…)` 测了物理前置,**真实消费者是在 `AddTenonAdminWorkflow` 之前调 `AddScoped`,那条路径有没有覆盖?**);⑥Plan §6 的 R1–R11 留白是否如实(尤其 **R5「未预料异常不 catch」的活锁风险**:`WfNodeExecutionDispatcher` 零 try/catch,handler 抛未预料异常会让 execution 行永远留在 `Running` 持租约直到租约过期再被领——这条要判定是「留白」还是已经够到 P2)。**仍不勾选**。
