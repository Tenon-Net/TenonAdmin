...
- 上一轮: Round 18 — Task 5 **exec**(executor/sonnet)。commit `cd356f4`,**4 个文件 +427/-0**,与 Plan §2 改动清单精确一致(新增 `Entities/WfOutbox.cs` 99 行、`Engine/WfOutboxStore.cs` 77 行、`tests/WfOutboxTests.cs` 217 行;修改 `Entities/WfEnums.cs` 34 行)。沿用文件交付法(`plan5.md` 给路径自读 + 报告落 `exec5-report.md`)。协调者**独立复核**:①**全 commit 零删除行**(`427 insertions(+)`,无 `deletions`)→ `WfEnums.cs` 纯追加要求自动满足,另单独验 `git show -- WfEnums.cs | grep -c '^-[^-]'` = **0**;②禁碰文件清单(`WorkflowSetup`/`ReplaceabilityTests`/`PersistenceContract`/`WorkflowAppFactory`/`docs/`/`web/`/`web-react/`/`.github/`/`.loop/`)在 commit 里命中 **0**,十件套 `[Fact]` 仍 **10** 条;③**三处可疑读数逐行深查**——`DefaultValue` 命中 2、裸 `ColumnDataType = "text"` 命中 1、`CodeFirst_BigString` 命中 2,乍看像违规,逐行核对后**全部落在 XML 注释里**(`:23`/`:26` 是 Plan 要求写明的「为什么全表不写 `DefaultValue`」、`:38` 是「禁止裸 text,SqlServer 非 Unicode 中文变 `???`」的警告、`:34` 是正文存全文的理由),**真属性行 9 个,`DefaultValue` 实际 0 命中**,唯一的 `CodeFirst_BigString` 属性在 `:76` 的 `PayloadJson` 上——合规;④store 侧同样两处可疑读数(`DateTime.UtcNow` 1、`catch` 2)**也全在注释里**,方法体逐行读过:`nowUtc` 走形参、**无 try/catch、无 `UseTran` 自开事务、签名无 `messageKey` 形参、不手赋 `Id`**;⑤实体核对:`: BaseEntity`(`:49`)、`uk_wf_outbox_message_key` 单列 + `IsUnique = true`、`idx_wf_outbox_scan(Status, AvailableAtUtc)`、`AvailableAtUtc` **非空**、`Status` 初始化器 `= WfOutboxStatus.Pending`、枚举 `1/2/3/4` **无 0 值**;⑥测试 7 条名字与 Plan §4 逐条对齐,禁写清单抽查(`Assert.True(true)`/`TODO`/`Skip =`)命中 **0**,`Queryable<WfOutbox>` 读回 8 处;**又一处可疑读数深查**:`Assert.Equal(WfOutboxStore.` 命中 1(疑似 Plan 点名禁止的「期望值由被测代码算出」),查实为 `:36` 把**输入常量**回读比对(同一常量在 `:26` 作入参),不是同义反复;T2 关键断言确实**手写字面拼接** `execution.ExecutionKey + ":" + messageType`(`:53`)——合规;⑦重跑两条闸门:build **0 错误**(13 警告全部是 Services/Core 既有 CS1573/CS1574,**Workflow 包自身贡献 0**;exec 自报「0 警告」是增量构建差异,非实情,同 Round 14 先例),test **320/320 通过、失败 0、跳过 0**(313 + 7 精确吻合)。
- 下一步: Round 19 — Task 5 **review**。协调者派 `Agent(model="opus"`,不传 `subagent_type`,给全工具含 Edit)。prompt 要求:明确 declare「自审」;**报告写进 `scratchpad/review5-report.md` 再回一句话**(消息通道有上限)。**本 Task 的变异着力点**——(a) `MessageKey` 构成:改成只用 `ExecutionKey`(丢 type 段)应让 **T2** 红、改成只用 `messageType`(丢 execution 段)应让 **T7** 红(这对偶的一对是 D2「第二种消息被静默吞掉」的守门测试,两条必须都在);(b) `AvailableAtUtc = nowUtc` 改成 `nowUtc.AddMinutes(1)`、初始 `Status` 写成 `Dispatching` → **T1** 应红;(c) 拿掉 `uk_wf_outbox_message_key` 的 `IsUnique = true` → **T4** 应红(注意 T3 抓不到这个,ensure 分支会提前返回,所以 T4 必须独立存在);(d) 把 ensure-insert 换成裸 `Insertable` → **T3** 应红;(e) `PayloadJson` 漏赋值 → **T5** 应红,把列从 `CodeFirst_BigString` 改成 `Length = 512` → **本机 SQLite 腿照绿**(射程限制 R5,如实确认而非硬凑);(f) 让 `EnqueueAsync` 内部自开 `UseTranAsync` → **T6** 应红(这是本轮对 §4.6「同一短事务提交」唯一的直接证据)。**另外重点审两处协调者已标记的疑点**:①**T2 的 `MessageKey` 断言打在 `EnqueueAsync` 的返回值上而非从库读回**(Plan §4 明写「凡是要证明落库了什么的断言必须重新读回」),判断这是不是真缺口——若 store 算对了 key 却落库落错,这条断言抓不抓得住?②**7 条测试是否有「写了但断言缺失」的空转**(Task 4 review 的教训:exec 避开了全部禁写类别却仍漏了两个核心行为,缺口是「断言缺失」不是「假断言」,**checklist 审查看不出来,只有变异测试能抓**)。手法照旧「先 grep 确认改了 → 跑测试 → 单文件 `git checkout` 复原」。**仍不勾选**。
