...
- 当前阶段: review 已完成,**0×P1 + 1×P2 待修**
- 上一轮: Round 15 — Task 4 **review**(Opus 自审,14 个变异点 = 10 必做 + **自加 4 个**)。**12 钉住、2 存活(同一条 P2)、1 个变异被判无效并换形状重做**。**无 P1**。P2-1:`StartedAtUtc`/`EndedAtUtc` 两个形参零覆盖(M11 互换、M12 写死,均 6 条全绿)。协调者独立交叉验证:①工作区干净、`git diff` 空、HEAD 未动 → 14 个变异全复原;②**刻意换手法**坐实 P2-1——不跑变异,改读断言与调用点:测试文件对这两列的 2 处命中**全在 `:241-242` 的 `NewAttempt` 构造辅助里、无一是断言**,且 6 个调用点**全部传相同的值两次**(`now, now` 等),连「开始 ≠ 结束」都从未被表达;③P3-1 复核确认三处独立字面量 512。**方法论亮点值得记牢**:子代理的 M10a 用 `CopyNew()` 逃出外层事务后一条没红、耗时却从 7s 飙到 5m4s,它没有就此报「#6 没鉴别力」,而是诊断出 SQLite 文件库上第二条连接撞写锁 → busy timeout 抛异常 → 同样触发回滚,**判定「变异无效」并换 M10b 重做**(方法内偷加 `CommitTranAsync`)后 #6 立刻红——区分「变异无效」与「测试没鉴别力」是变异测试里最易糊弄的一步。
- 下一步: Round 16 — Task 4 **修 Findings + 勾选**(按 Round 8/12 先例合并)。协调者派 `Agent(subagent_type="oh-my-claudecode:executor", model="sonnet")`,**只修 P2-1 一条**(P3-1 可顺手改成引用 `SummaryMaxLength` 常量,**但不强制**;P3-2 与射程限制一律不动)。修法已定:在测试 #1 里让两个时刻取**不同值**(`var ended = started.AddSeconds(3);`,**必须不等否则互换仍测不出**),再加两行 `Assert.Equal` 断 `StartedAtUtc`/`EndedAtUtc`;断内存对象、不经 DB 往返。**不新增测试方法**(条数仍 313),**产品代码零改动**。子代理跑完后协调者**亲自重跑 M11/M12 确认转红后复原**(且**换一种形状**,别照抄子代理的:例如把两列同时写成 `startedAtUtc`、或让 `EndedAtUtc = startedAtUtc.AddSeconds(-3)` 造负耗时),再重跑两条闸门(期望 313/313)。0×P1、0×未修 P2、闸门已跑 → **勾选 Task 4**,Status 写「下一步 = Round 17 Task 5 plan(`wf_outbox` 实体 + 可靠派发骨架)」。
