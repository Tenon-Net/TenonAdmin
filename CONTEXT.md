...
| 术语 | 定义 |
|---|---|
| 工作流 | 本仓语境 = **人工审批链**(等人点同意),不是自动化编排(Elsa/Conductor 那类)。产品定位:审批为核。卫星包 `TenonAdmin.Workflow`,不进内核。 |
...
| Webhook 节点 | 唯一内置的自动化节点类型:流转到达时调用消费者配置的 HTTP 地址,结果写回流程变量。是"审批为核"里保留的最小自动化种子。 |
| 节点类型 SPI | 节点类型体系可扩展的约定:内置类型之外,消费者可注册自定义节点类型而不改内核。完整自动化编排(脚本/定时/重试策略)明确不做。 |
