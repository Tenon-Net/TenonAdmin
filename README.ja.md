...
- **依存関係を押し付けない** — ランタイムは SqlSugar + `Microsoft.*` のみ。Redis、MQTT、SM2/SM3/SM4 暗号は、必要になったときだけオプションパッケージで追加します。
...
M3 完了 — バックエンドカーネル、全フロントエンド管理ページ、設定センターが動作しています。現在のバージョン: `0.0.1-preview`。
