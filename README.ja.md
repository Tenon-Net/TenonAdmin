...
初回リリースに必要な機能は揃っています — バックエンドカーネル、全管理画面、設定センター、コンテナ配布、マルチレプリカ対応（Redis キャッシュ、レプリカ間で共有されるレート制限カウンタ、レプリカごとの Snowflake ワーカー ID）がいずれも動作し、CI でカバーされています。現在のバージョン: `0.0.1-preview`。

nuget.org にはまだ未公開です。現時点ではソースからの実行をお願いします。1.0 までに API が変わる可能性があります。開発は `dev` ブランチで行っています。
