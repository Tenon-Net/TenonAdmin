...
- **认证**——账号密码 + 验证码，JWT + 刷新令牌轮换，登录锁定，在线会话与强制下线；可选 TOTP 自助绑定 / Cookie 会话（默认关闭，见 `docs/agents/security-optional-config.md`）。**不是**等保定级产品
...
- **定时任务**——内核自带的调度器，不引新依赖也不用额外进程：cron（6 段秒级，含 `L`/`W`/`#`）、固定间隔、一次性三种触发；载荷写一个 `IAdminJob` 类即可，也可以完全不写代码、直接配一次 HTTP 调用。多副本经数据库租约选主，主副本挂了备副本接管，而同一次触发全集群只执行一次
