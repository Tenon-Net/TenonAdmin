...
/// </summary>
public class LoginLockService(ICacheProvider cache, ISecurityPolicyProvider policy) : ILoginLockService
{
    /// <summary>
    ...
    {
        var (maxFail, lockMinutes) = await policy.GetLoginLockAsync();
        if (maxFail <= 0) return;   // 功能关闭
        ...
        AdminException.ThrowIf(count >= maxFail, ErrorCode.AccountLocked,
            new Dictionary<string, object?> { ["lockMinutes"] = lockMinutes });
    }
    ...
    public virtual async Task RecordFailureAsync(string account)
    {
        ...
        await cache.IncrementAsync(CacheKeys.LoginFail(Normalize(account)), TimeSpan.FromMinutes(lockMinutes));
    }
