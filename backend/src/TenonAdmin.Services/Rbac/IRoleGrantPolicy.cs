...
/// <summary>
/// 角色授予策略——集中"当前用户能否把这些角色新授予给这个用户"的判定,
...
/// <para>
/// 判定规则:超管(<see cref="TenonAdmin.Core.ICurrentUser.IsSuperAdmin"/>)或系统/未认证上下文(与
/// <see cref="TenonAdmin.Core.IDataScopeContext"/> 同一"未显式设置=可信"约定)不受限;其余用户只能把
/// "启用中 + 未删除 + <c>IsDelegatable == true</c>"的角色,授予其当前数据范围内的用户,
/// 违反分别抛 <see cref="TenonAdmin.Core.ErrorCode.RoleNotDelegatable"/> /
/// <see cref="TenonAdmin.Core.ErrorCode.UserOutOfDataScope"/>。
/// </para>
/// <para>
...
public interface IRoleGrantPolicy
{
    ...
    /// 不满足直接抛 <see cref="TenonAdmin.Core.AdminException"/>;<paramref name="addedRoleIds"/> 为空时恒通过(无新增即无需校验)。
    /// </summary>
