...
/// 未配号时不随机、不哈希：在 <c>sys_worker_lease</c> 上 INSERT 空闲槽，谁成功谁用谁。
/// hostname 哈希 / 随机 0–63 会静默撞主键（2 副本约 1.6%、4 副本约 9%），明确不做。</para>
/// </summary>
public class WorkerIdGuardTests
{
    ...
    [Fact]
    public void Redis_cache_without_an_explicit_worker_id_auto_claims_a_slot()
    {
        using var f = Factory(
        ...
        Assert.Equal(0, f.Services.GetRequiredService<WorkerIdAssignment>().WorkerId);
    }
