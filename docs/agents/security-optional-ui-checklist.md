...
默认 **TOTP / CookieMode 全关**。TOTP 总闸与登录验证码一样走 **配置中心**（即时生效，一般不必改 appsettings）：

1. 登录超管 → **系统配置 → 安全策略**
2. 打开 **启用动态口令** → 保存  
3. （可选）打开 **超管必须动态口令**
4. 用户管理里对目标用户打开 **强制动态口令**

| 想测什么 | 怎么开 |
|----------|--------|
| 绑定 / 恢复 / 登录 TOTP | 配置中心 `sys.security.totp.enabled` |
| 超管必须二因子 | 同上 + `sys.security.totp.requireForSuperAdmin` |
| 指定用户必须二因子 | 用户编辑 **强制 TOTP**（须先开总闸） |
| Cookie + CSRF 会话 | appsettings `Session:CookieMode=true`（仍属部署级）后**重启后端** |

已有库若缺种子行：重启一次后端让 `ConfigSeed` 补 `Id=29/30`；或手工加同名键。
...
| 能力 | 入口（UI） | 路由 / 页面 | 依赖配置 |
|------|------------|-------------|----------|
| 强制 MFA 未绑定引导 | 账密登录返回 **40020** 后，**Modal**「设置身份验证器」 | `/mfa/bind?account=` | Totp 开 + 强制对象且未绑 |
| 个人安全中心（自愿绑定） | 顶栏头像下拉 → 账号安全 | `/personal/security` | 无；链到 bind 才需 Totp |
| 自助绑定 / 恢复码 UI | 个人安全，或 40020 引导 / TOTP 步「使用恢复码」 | `/mfa/bind`（可 `?account=` / `?mode=recovery`） | `Totp:Enabled` |
| 强制某用户 MFA | 系统 → 用户 → 编辑 | 表单「强制 TOTP」 | 写用户权限 |
| 登录 TOTP 挑战 | 账密登录后（已绑定） | 登录二步（40018） | Totp 开 + 用户已绑且被强制/策略要求 |
...
**登录页默认不展示**「设置身份验证器」——内网多数部署不开 MFA，避免常驻入口误导。自愿绑定走登录后 **账号安全**。
...
| # | 页面 / 操作 | 预期 | 勾选 |
|---|-------------|------|------|
| A1 | 打开登录页 | 账密可登录；**无**常驻「设置身份验证器」链接 | ☐ |
| A2 | 直开 `/mfa/bind`（Totp 关）尝试绑定 | 失败/无权限，**不**白屏、不卡死 | ☐ |
| A3 | 登录后顶栏用户下拉 | 有 **账号安全**（中/英随语言） | ☐ |
| A4 | 进入账号安全 | `/personal/security`：标题、说明、两按钮 | ☐ |
| A5 | 从个人安全点「设置身份验证器」 | 跳到 `/mfa/bind`（宜带当前 account query） | ☐ |
| A6 | 系统 → 用户管理 | 列表正常；编辑有 **强制 TOTP** 开关 | ☐ |
| A7 | 用户列表「清除二因子」 | 仅当行上用户已绑 TOTP 时出现；关 Totp 时 clear 应失败 | ☐ |
...
| # | 操作 | 预期 | 勾选 |
|---|------|------|------|
| B1.1 | 顶栏 → 账号安全 → 设置身份验证器 | `/mfa/bind`（宜带 account） | ☐ |
...
### 4.2 强制 MFA 未绑定（登录 40020 引导）

| # | 操作 | 预期 | 勾选 |
|---|------|------|------|
| B2.1 | 用户编辑打开 **强制 TOTP**，该用户**未绑** | 保存成功 | ☐ |
| B2.2 | 该用户账密登录 | **不**直通；弹出 **Modal**（账密表单仍在底下，非 toast、非整卡替换） | ☐ |
| B2.3 | 点「设置身份验证器」 | 关 Modal，进 `/mfa/bind`，query 带 account | ☐ |
| B2.4 | 完成绑定后回登录再登 | 进入 TOTP 步或（若策略已满足）正常登录 | ☐ |
| B2.5 | 登录页默认态（未触发 40020） | **仍无**常驻绑定链接 | ☐ |

### 4.3 登录二次因子（已绑定）

| # | 操作 | 预期 | 勾选 |
|---|------|------|------|
| B3.1 | 强制 TOTP 用户已绑后账密登录 | 密码过后进入 TOTP 步（40018） | ☐ |
| B3.2 | 正确 TOTP | 进入布局壳 / 首页 | ☐ |
| B3.3 | 错误 TOTP | 失败可重试 | ☐ |
| B3.4 | TOTP 步可见「使用恢复码」 | 跳 `/mfa/bind?mode=recovery`（可带 account） | ☐ |
| B3.5 | （可选）`RequireForSuperAdmin=true` 且超管未绑 | 同 B2.2 引导面板，**不是**部署 InitGrant | ☐ |
