...
- [x] **C12 dashboard + personal 七页** — 拆两半。**C12a dashboard `624de58` 已落**(workbench/biz,菜单驱动,见轮次日志)。**C12b personal 五页待落**(scope 已全核,下条即做):
  ...
  - **顶栏接线**(LayoutShell 现只有折叠/主题/登出,缺 Vue AppHeader 那套):加**用户下拉**(头像+名 → profile/password/sessions/bindings + 登出,替换裸登出钮)+ **通知铃铛**(unreadCount 轮询角标 → /personal/notice)。参考 Vue `layouts/AppHeader.vue` + `NoticeBell.vue`(开工再读)。
  - **纯逻辑变异钉**:password confirm 匹配校验、profile initial(name) 首字母、profile load/save 字段映射(仿 C11 结构化面板 spec 守字段映射);sessions/bindings 接线为主。
  - C12b 完再开**合并 C12 review lane**(dashboard + personal 一并审,照 C10 批量审先例)。
...
四脚本都加 web-react(5174):dev.bat 起第三个窗口、dev.sh 起后台 job(`.dev/web-react.log`/`.pid`)、stop.bat/stop.sh 端口循环加 5174 + 清 pid。两前端 vite 都读 `TENON_API_TARGET`(dev.sh 已 export)反代同一后端 5100。`bash -n dev.sh/stop.sh` 语法通过;.bat 简单未另验。**下一条**:**E4 文档**(根 CLAUDE.md 加 web-react 段、写明两模板各自自包含零共享且刻意、不出现"必须一起带上";site/ 加一页 React 模板上手 degit 一条命令)。之后 E5 gen:api 漂移闸、E6 pro-components 转正,最后批次 F。
