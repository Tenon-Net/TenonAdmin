...
TenonAdmin 前后端同仓、**同版本号一起发**。后端以 NuGet 包发布(版本由 tag 的 `-p:Version` 注入),前端版本号是登录页页脚的构建期常量(`web/package.json` 与 `web-react/package.json`,两套模板各一份)——必须一起 bump,否则用户看到的版本和实际跑的包对不上。
...
3. **本地验绿**(CI 会再跑一遍,但先本地确认省得白打 tag;本机内存紧,**别并发**跑 vue-tsc 和 dotnet test,一次一个):
   ...
   ```bash
   cd web && npm test && npm run build              # Vue 模板:单测 + 类型检查 + 打包
   cd web-react && npm test && npm run build        # React 模板:同上(注意 cwd 回仓库根再进)
   dotnet test backend/TenonAdmin.slnx -c Release   # 后端:默认 SQLite(exit 0 即全绿)
   ```

4. 提交:`chore(release): prepare X.Y.Z — changelog + frontend version bumps`。
