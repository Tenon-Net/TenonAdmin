...
当前交付的是这条可靠执行内核及节点 handler SPI：execution 的稳定身份、领取租约、fence、attempt、结果分类和终态 outbox 都在同一条装配链上。远程 handler 不持有工作流数据库事务；引擎只接收显式结果并负责状态推进。Task 8b 已把 Webhook 接入真实流程入口和既有 `IAdminJob` worker；Webhook 是可替换的首个真实 handler，Fake handler 用于契约验证，AI 仍只是未来可接入的 Adapter，不属于内核本身。

这是已经接入生产流程入口的 worker 闭环：`EnterNodeOp` 在工作流事务内创建/复用 `wf_node_execution`，`WfNodeExecutionJob` 通过固定 `sys_job` 行扫描并调用 dispatcher。该 worker 仍只负责 execution 扫描，不消费 outbox。Task 8c 已补齐 `WfOutboxJob`/`WfOutboxDispatcher`/`IWfOutboxTransport`；M3a-2 Vue 已交付 Webhook 设计器 UI。实现锚点为 [`WfNodeExecutionDispatcher`](../../backend/src/TenonAdmin.Workflow/Engine/WfNodeExecutionDispatcher.cs)（第 6–101 行）、[`WorkflowEngine.BeginNodeExecutionCompletedAsync`](../../backend/src/TenonAdmin.Workflow/Engine/WorkflowEngine.cs)（第 1182–1314 行）、[`WfNodeExecutionJob`](../../backend/src/TenonAdmin.Workflow/Jobs/WfNodeExecutionJob.cs)、[`WfOutboxDispatcher`](../../backend/src/TenonAdmin.Workflow/Engine/WfOutboxDispatcher.cs) 和 [`WorkflowSetup`](../../backend/src/TenonAdmin.Workflow/WorkflowSetup.cs)。
...
重提不是旧自动节点的下一次 attempt：它复用 token，但会从 start 创建新的 `NodeVisitId`。为与完成 tx2 的 `execution → token` 锁获取顺序一致，引擎在同一事务内先取消该 token 上所有活跃 execution（`Pending`/`RetryScheduled`/`Running`）、清空 lease/retry 并递增 Fence，再做 token version CAS；CAS 失败会回滚先前 invalidation，成功才进入新 start。旧 handler 的迟到 tx2 因 execution 已非 `Running` 且 Fence 已变而整体回滚，不写 attempt/outbox，也不推进新 visit；完成回写还核验当前 token 的 instance/node/node visit 身份，以抵挡未来的其他 token 重定位路径。

如果领取后的 instance、token、definition version 或模型/节点快照被确认永久破坏，dispatcher 会发出内部 quarantine command。该命令仍进入引擎 tx2，但只做旧 `Fence + Running` CAS、清 lease、追加 terminal attempt 和 `Pending` outbox，不再读取缺失上下文，也不推进 Token；瞬时数据库/基础设施错误不走 quarantine，仍保留 lease 过期后的恢复机会。

attempt 只追加不更新/删除，`AttemptNo` 直接取领取后的 `AttemptCount`。每个 handler 已返回的结果，或 dispatcher 在缺少 handler 时合成的结果，各落一行 attempt；handler 在返回结果前崩溃或外部取消时 tx2 不启动、不写 attempt；未知非取消异常则由 dispatcher 合成受控 `RetryableFailure/48032`，因此会写入一条有限重试 attempt。outbox 的 `MessageKey` 为 `{ExecutionKey}:{MessageType}`，有唯一索引并由 `WfOutboxStore.EnqueueAsync` ensure-insert；`WfModelJson.Options` 的 `WhenWritingNull` 会省略空 payload 属性，不会序列化成 `null`。`WfHistory.PayloadVersion` 是永久语义：对仍在做父表迁移、首次加入该列的环境，`SugarColumn(DefaultValue = "0")` 在加列/回填阶段给存量行 legacy 0；如果父表迁移已经加过该列，之后再补写 `DefaultValue` 不会追溯性地应用，也不能借此重写旧 history。新建 `WfHistory` 行由 CLR 初始化器得到 1；读取必须按 `EventType + PayloadVersion` 解释，不能改写 append-only 的旧 history。对应源码为 [`WfNodeExecutionAttemptStore`](../../backend/src/TenonAdmin.Workflow/Engine/WfNodeExecutionAttemptStore.cs)（第 27–66 行）、[`WfOutboxStore`](../../backend/src/TenonAdmin.Workflow/Engine/WfOutboxStore.cs)（第 21–64 行）、[`WfHistory`](../../backend/src/TenonAdmin.Workflow/Entities/WfHistory.cs)（第 80–87 行）和 [`WfModelJson`](../../backend/src/TenonAdmin.Workflow/Schema/WfModel.cs)（第 41–69 行）。

`WfOutboxStatus` 依次为 `Pending`、`Dispatching`、`Dispatched`、`Failed`。Task 8b 仍只实现终态结果幂等入队为 `Pending`。Task 8c 已落地领取、可见性超时重领、退避、`Dispatched`/`Failed` 回写、`Failed → Pending` 人工重放和 `wf-outbox-scan` 后台扫描；execution worker 仍不读取或消费 `wf_outbox`。状态定义见 [`WfEnums.cs`](../../backend/src/TenonAdmin.Workflow/Entities/WfEnums.cs)，实体、入队与消费实现见 [`WfOutbox.cs`](../../backend/src/TenonAdmin.Workflow/Entities/WfOutbox.cs)、[`WfOutboxStore.cs`](../../backend/src/TenonAdmin.Workflow/Engine/WfOutboxStore.cs)、[`WfOutboxConsumerStore.cs`](../../backend/src/TenonAdmin.Workflow/Engine/WfOutboxConsumerStore.cs) 和 [`WfOutboxDispatcher.cs`](../../backend/src/TenonAdmin.Workflow/Engine/WfOutboxDispatcher.cs)。
...
该切片从 Provider 到流程落库始终是 shadow-only：AI 不自动批准、拒绝、完成 task 或推进 token，合法低风险 proposal 也会进入人工兜底。M3b-0 交付时只保证 Task 8b 的 `Pending` outbox 幂等入队；Task 8c 的 consumer/transport 已于 2026-09-14 补齐，不改变 shadow-only 边界。M3B0-17 至 M3B0-20 及后续审查修复已经完成，最终聚焦矩阵 326/326 通过，代码与架构复核无 blocker。
...
| M3a | 已交付可靠自动节点执行内核、Task 8b 生产 Webhook 闭环与 Task 8c outbox consumer：稳定 `ExecutionKey`、`WfNodeExecution` 状态机、lease/fence claim、append-only attempt、结构化结果、终态 `Pending` outbox、dispatcher、`IWorkflowNodeHandler` SPI、`EnterNodeOp` 入口、`IAdminJob` execution worker，以及 `IWfOutboxTransport`/`WfOutboxDispatcher`/`wf-outbox-scan`。 | 目标 E2E 与本地 Release 验收已证明 execution 闭环；Task 8c 本地 SQLite 聚焦 `68/68`、相关回归 `180/180`；M3a-2 Vue 设计 UI 已交付，React port 仍后置。 |
| M3b-0 | 已通过 M3a SPI 接入 `AI Decision Adapter`：OpenAI-compatible/fake provider、结构化 proposal、schema/policy、shadow-only、人工 fallback、审计和限额。 | Round 19 已完成目标验证；受控自动化仍后置，AI 不能直接取得审批权。 |
