...
种子按**两个应用**拆分受众(`WorkflowMenuSeed`,schema ≥ 6):

- **系统 → 流程管理**(管理员):定义列表 / 设计器 / 流程监控 / 长期委托
- **业务中心 → 审批中心**(员工自助):发起流程 / 待我审批 / 我发起的 / 我已办的 / 抄送我的(`[ActiveSession]` 端点)
- 审批详情:进度时间线 + 意见记录 + 业务表单挂载点 + 操作按钮组(两应用共用路由 `/workflow/instance/{id}/detail`)

普通员工角色只授业务中心下的审批菜单即可,无需进入「系统」应用。
