...
Composition API mode (`legacy: false`), default locale `zh-CN`, falling back to `en-US` for any key missing from the active locale. The entire translation surface is two files — `locales/zh-CN.ts` and `locales/en-US.ts` — with no per-component splitting and no lazy loading.
