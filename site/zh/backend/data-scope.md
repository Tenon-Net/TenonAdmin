...
翻成大白话：范围写入的地方是授权过滤器，读取的地方是更靠后的业务动作，`AsyncLocal` 只顺着执行流往下走，写在过滤器这层的值，传不到后面的动作里。`HttpContext.Items` 不吃这一套，它挂在请求对象本身上，写一次，全管道随时读得到同一份，所以这里必须选 `Items`，不能用 `AsyncLocal`。
