...
真实现在 `AddTenonAdminServices()` 之前用 `TryAddSingleton` 注册，压过 Services 层的 Noop。消费方想换自己的 `IRealtimePublisher`，比如推给 MQ 而不是 SignalR，照旧在 `AddTenonAdmin()` 之前注册，先到者胜：
...
客户端里的路径是**写死的** `` `${baseUrl}/hub/realtime` ``。后端把 `TenonAdmin:Realtime:HubPath` 改成别的，前端连不上，却也不会报错，只会当成「实时没开」，静默退回轮询。要改路径，就得两边一起改。
:::
