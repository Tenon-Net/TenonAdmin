...
本页梳理 `backend/` 的解决方案结构、`src/` 下六个包各自的定位、示例宿主如何启动,以及测试套件如何同时验证"裸内核"和"带自定义业务模块的消费方"两种场景。架构设计的"为什么"(依赖方向、可替换性、请求管道)见[架构](/zh/backend/architecture)——本页讲结构与启动,那页是深入解读。
...
- **`TenonAdmin.TestHost`** 是一个最小化的*消费方*宿主,不属于自动化测试套件本身——它把自己登记进 `options.ApplicationAssemblies.Add(typeof(Program).Assembly)`,让自己的实体、种子数据、控制器(`SampleWidget`、`SampleDoc`、`CustomDictController`)走一遍消费方挂载路径,供基于 `WebApplicationFactory<Program>` 的测试驱动。它演示的内容详见[搭建业务模块](/zh/tutorial/business-module)。
...
- 构建、测试、运行后端的 CLI 命令 → [贡献指南](/zh/community/contributing)
- 包职责与 DI 装配的深入解读 → [架构](/zh/backend/architecture)
