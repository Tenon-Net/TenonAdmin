...
这页收的是真正的"第一问"——装完包、第一次把内核跑起来时最容易卡住的那几件事。别的问题大多藏在报错原文里:内核对可预期的配置错误(建表闸门、`WorkerId` 缺配等)会点名到具体的配置项/表/列,而不是甩一句笼统异常,顺着它读通常比翻文档快。真读不出来,再去仓库 [issue](https://github.com/Tenon-Net/TenonAdmin/issues) 搜关键字;开新 issue 时把 .NET / Node 版本、`TenonAdmin:Database:DbType`、单实例还是多副本、完整报错堆栈一并带上,能省一轮来回。
...
想固定成自己的密码(CI、自动化场景),启动前配 `Seed:AdminPassword` 即可:
...
它被 `.gitignore` 排除了,不在版本库里——这个文件放的是本地凭证(数据库连接串、JWT 密钥),不该进 git。从旁边的 `appsettings.Development.json.example` 拷一份改名即可,样例宿主在 `backend/samples/MinimalHost/appsettings.Development.json.example`,按需改。
