...
想给后台换个主色、补一套暗色配色，或者塞几个自己的图标，要动的地方比想象中少。外观全部由一层 CSS 变量驱动，图标在启动时一次性离线注册——这页讲清这两套机制，好让你知道改哪里、以及为什么只改那里。
...
`app` store（`web/src/stores/app.ts`）的 `themeScheme` 有三态：`'light'`、`'dark'`、`'auto'`（默认）。`auto` 下，`isDark` getter 通过 VueUse 的 `usePreferredDark` 响应式读取系统偏好——首访自动匹配系统深浅色，系统主题变了也实时联动。点顶栏的切换按钮（`toggleDark()`）或调 `setThemeScheme()`，会落到明确的 `'light'`/`'dark'`，并随 store 持久化进 `localStorage`(key `app`)，此后不再跟随系统。
...
Iconify 数据和 SVG 都打进应用本身，图标渲染从不请求外部 CDN（如 `api.iconify.design`）——离线或网络受限的部署环境里，表现和联网时一致。
...
`IconPicker`（`web/src/components/IconPicker/index.vue`）是应用里的选择器，用在**系统管理 → 菜单管理**的菜单 `icon` 字段上。它封装 npm 包的 `IconPicker`，注入 tenon 自己的 vue-i18n 文案，并复用 `setupIcons()` 已全局注册好的图标集（所以 `ph` 会作为首个/默认 Tab）——调用处不用再配置。

::: tip 选择器完整 API
这里只讲图标在应用里怎么接入。选择器组件本身——多图标库 Tab、注册本地 SVG、`labels`/i18n、`v-model` 约定——由独立包提供，API 见 [IconPicker](/zh/components/icon-picker)。
:::
