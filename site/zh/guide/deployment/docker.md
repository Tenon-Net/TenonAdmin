...
Caddy 在前面挡着，不解析 `X-Forwarded-For` 的话，后端看到的永远是 Caddy 那一个 IP。于是全体用户共享一个限流桶，一人爆破登录就能锁死所有人，登录日志里的 IP 也全是代理地址。多副本更是每个副本都要配，否则都只看得见负载均衡器那一个 IP。打开这项就**必须同时声明受信来源**，这里是 Docker 桥接网段。受信的是来源地址，所以任何能直连后端端口的人都能伪造 IP。`app` 的端口映射只绑 `127.0.0.1`，正是为了这个：
