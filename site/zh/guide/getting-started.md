...
这页带你把 TenonAdmin 内核跑起来,拿到一个能用的令牌调通第一个受保护接口,再把它接进你自己的 ASP.NET Core 项目、起好前端模板、换掉默认数据库。全程零配置,不用先装数据库。
...
种子只在 `sys_user` 表为空时跑一次。跑 MinimalHost 属于零配置启动,没有显式配密码,内核就生成一个 16 位随机密码(用加密安全随机源,剔除了 `0/O`、`1/l/I` 这类易混淆字符),在**建号那一次**启动的控制台日志里用一个边框醒目打印出来,仅此一次:
...
要跨副本共享会话和缓存(多实例部署),额外装 `TenonAdmin.Caching.Redis`,并在 `AddTenonAdmin` **之前**调 `AddTenonAdminRedisCache(builder.Configuration)`——内核的可替换服务都用 `TryAdd` 注册,谁先注册谁赢,晚于 `AddTenonAdmin` 就抢不过内置的进程内缓存了。没配 `Cache:Provider=Redis` 时这行是空操作,单实例开发不受影响。
...
::: warning 生产不会自动建表
`ASPNETCORE_ENVIRONMENT=Production` 时,即便开了 CodeFirst 也**不会**自动建表——这是防止线上误改表结构的安全闸门。空库首次上生产,要么临时打开 `EnableCodeFirstInProduction: true` 让它自己建一次,要么让 DBA 手工建。详见[部署指南](/zh/guide/deployment/)。
:::
