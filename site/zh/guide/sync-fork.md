...
自己的二次开发放在 `my-product` 上，或者它下面再分的分支，不要直接写在 `main`、`dev` 这些还要拿来拉上游的同名分支上。分开放，将来合并出问题时，你不用再从自己的提交历史里把它摘出来。
...
- **`web/src/api/schema.d.ts`**：特例，永远不要合并它，重新生成它。它是 6000 行的生成物，而你的后端有你自己的控制器，所以你这份从第一天起就和上游 100% 分叉。上游一动，它就是整文件冲突。别手动解：
