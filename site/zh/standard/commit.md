...
两者面向的读者不一样。代码和文档是给维护这个仓库的人看的,团队内部沟通用中文效率更高、更准确。而提交历史(`git log`)是仓库对外的一部分 —— TenonAdmin 是面向所有 .NET 使用者分发的 NuGet 包,commit 历史会被贡献者、下游消费方、自动化工具(release note 生成、语义化版本判定)读取,统一用英文是让这条历史对所有人可读、可被工具解析。
...
- `scope` 可以是单个模块(`web`、`notice`、`menu`),也可以逗号并列多个(`web,backend`)——改动确实横跨两侧时如实标注,不必强行归到一个。
...
正文不是用来复述 diff 的(diff 本身就能看到改了什么代码),而是补充 diff 里看不出来的背景。日常的小修小补(样式调整、单个 bug 修复)不需要正文,一行 `subject` 足够。
...
- 把多个不相关的改动塞进一个 commit,再用一个笼统的 `type` 概括(比如同时改了 `feat` 的功能和 `fix` 的缺陷)——按语义拆成多个提交。
