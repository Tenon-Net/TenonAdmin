...
两者面向的读者不一样。代码和文档是给维护这个仓库的人看的,团队内部沟通用中文效率更高、更准确。而提交历史(`git log`)是仓库对外的一部分 —— TenonAdmin 是面向所有 .NET 使用者分发的 NuGet 包,commit 历史会被贡献者、下游消费者、自动化工具(release note 生成、语义化版本判定)读取,统一用英文是让这条历史对所有人可读、可被工具解析。
