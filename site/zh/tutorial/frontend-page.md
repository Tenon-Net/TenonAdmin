...
接着上一篇[端到端加一个业务模块](/zh/tutorial/business-module)——后端已经有了 `GET/POST/PUT/DELETE /api/v1/sample/doc` 这组接口。本篇把它做成一个能操作的管理页面。
...
页面能跑之后,想把整套系统发布出去,看下一篇:[容器化部署一条龙](/zh/tutorial/docker-deploy)。
