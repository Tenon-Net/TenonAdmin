...
/**
 * Level3 强制 TOTP 登录 UI:密码通过后 40018 → 输入动态口令 → 进门户。
 * 后端 40018 策略依赖 Level3;此处 mock 信号,专测两端登录页完成流。
 */
test('login TOTP challenge (40018) completes and enters app', async ({ page }) => {
  ...
  expect(loginHits).toBeGreaterThanOrEqual(1)

  await page.getByPlaceholder(/动态口令|6.?位|totp/i).fill('123456')
  await page.getByRole('button', { name: /登\s*录|sign\s*in|提交|submit/i }).click()
