...
describe('roleToInput', () => {
  it('全量映射每字段(remark 归一空串)', () => {
    expect(roleToInput(ROLE)).toEqual({ code: 'admin', name: '管理员', sort: 3, enabled: true, remark: '', isDelegatable: false })
  })
  it('remark 有值时保留', () => {
...
describe('blankRole', () => {
  it('默认:空 name/code、sort 0、启用、空 remark', () => {
    expect(blankRole()).toEqual({ name: '', code: '', sort: 0, enabled: true, remark: '', isDelegatable: false })
  })
})
