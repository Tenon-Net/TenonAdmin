...
const filled: UserForm = {
  ...
  gender: 'M', enabled: true, forceTotp: true, totpEnabled: false, roleIds: [1, 2],
...
describe('toAddInput 空值语义', () => {
  ...
  it('roleIds / enabled / forceTotp / gender 原样透传', () => {
    const r = toAddInput({ ...filled, roleIds: [5, 6], enabled: false, forceTotp: true, gender: 'F' })
    expect(r.roleIds).toEqual([5, 6])
    expect(r.enabled).toBe(false)
    expect(r.forceTotp).toBe(true)
    expect(r.gender).toBe('F')
  })
})

describe('toUpdateInput 全量替换:透传字段不能丢', () => {
  it('orgId/positionId/directorId/avatar/forceTotp 原样回送(本页无编辑控件但必须带回,否则被后端置空 = 数据丢失)', () => {
...
describe('detailToForm 回显', () => {
  const detail: UserDetail = {
    ...
    enabled: false, isSuperAdmin: false, forceTotp: true, totpEnabled: true,
    roleIds: [3], createTime: '2026-01-01',
  }
  it('可空文本 null → 空串(绑 input);password 永远空串;透传字段带出', () => {
    ...
    expect(f.forceTotp).toBe(true)
    expect(f.totpEnabled).toBe(true)
  })
...
describe('blankForm', () => {
  it('默认 enabled=true、forceTotp=false、空角色、透传字段全 null', () => {
    const b = blankForm()
    expect(b.enabled).toBe(true)
    expect(b.forceTotp).toBe(false)
    expect(b.totpEnabled).toBe(false)
