...
describe('workflow form schema', () => {
  ...
  it('drops maxSelected when a user field turns multiple off', () => {
    const capped = updateWfFormFieldProp(
      updateWfFormFieldProp(createWfFormField('user', 'owner'), 'multiple', true),
      'maxSelected',
      3,
    )
    const single = updateWfFormFieldProp(capped, 'multiple', false)

    expect(capped.props).toEqual({ multiple: true, maxSelected: 3 })
    expect(Object.keys(single.props ?? {})).toEqual(['multiple'])
    expect(validateWfFormSchema({ version: 1, fields: [{ ...single, label: '负责人' }] })).toEqual([])
  })
