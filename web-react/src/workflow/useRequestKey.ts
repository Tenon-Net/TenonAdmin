...
//                 让服务端的回执兜住"其实已经成功了"的情况——这是 receipt 存在的意义,不是本函数发明的)。
//   reset()   —— 无条件丢弃,给"打开一个新动作"用(不依赖 settle 是否发生过)。
// 页面级工厂,不是 store:请求键的生命周期天然绑定单个页面/单个弹窗实例。
// 刻意不用 useRef:状态不驱动渲染,且单测可直接调用(与 Vue 侧 composable 用法对齐)。
...
export function useRequestKey() {
  ...
  function value(): string {
    if (key == null) key = generateId()
    return key
  }

  function settle(outcome: RequestKeyOutcome) {
    if (outcome !== 'network') key = null
  }
